...
if __name__ == '__main__':
    ...
    opt.print_freq = max(1, dataset_size // opt.batch_size // opt.print_freq * opt.batch_size)
    opt.display_freq = max(1, dataset_size // opt.batch_size // opt.display_freq * opt.batch_size)
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(pbar):  # inner loop within one epoch
            ...
            if total_iters % opt.print_freq == 0:    # print training losses and save logging information to the disk
                ...
                message = visualizer.print_and_get_loss_message(epoch + float(epoch_iter) / dataset_size, losses, optimize_time, t_data)
